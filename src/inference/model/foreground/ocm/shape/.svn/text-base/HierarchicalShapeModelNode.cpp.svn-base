...
// 	mVisualizer->setLearningSamples(mRawData);
  ...
  double HierarchicalShapeModelNode::calculateProbabilityForHypothesis(std::vector<pbd_msgs::PbdObject> pEvidenceList, std::vector<unsigned int> pAssignments, unsigned int& pSlotId, bool pCut)
  {
    double result = 1.0;
...
//       ROS_DEBUG_STREAM("Pose fitting report for scene object '" << mSceneObject <<"'. Position is " << scorePos << ", orientation is " << scoreOri << ". Total is " << score << ".");
  ...
  void HierarchicalShapeModelNode::visualize(std::vector<pbd_msgs::PbdObject> pEvidenceList)
  {
    // Try to find evidence for this scene object.
    for(unsigned int i = 0; i < pEvidenceList.size(); i++)
    {
      // Get the object.
      pbd_msgs::PbdObject object = pEvidenceList[i];
